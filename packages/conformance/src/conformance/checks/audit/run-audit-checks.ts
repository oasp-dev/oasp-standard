...
/**
 * Drives a `ReferenceServer` through all six required-emission
 ...
 * per § Scope provenance's total four-row table: `publish` →
 * definition scope, `migrate` → conversation scope, the four
 * session-bound interactions → the bound Conversation's scope, or —
 * for a Session with no bound Conversation — the pinned
 * `AgentDefinition`'s scope.
 *
 * **Deliberately not checked here (documented v0 gap, not an
 * oversight):** initial Conversation/Session creation — including its
 * credential/`vaultIds` attachment — is not audited in v0. Per
 * `docs/spec/audit.md` § The credential-attach gap, there is no
 * `create*` interaction in the required-emission set to check against.
 * This function calls `server.createConversation` purely as scenario
 * setup and never asserts an `AuditEvent` results from it — adding
 * such an assertion would encode a requirement the standard does not
 * (yet) impose.
 */
export async function runAuditChecks(server: ReferenceServer): Promise<CheckResult[]> {
  ...
  ); // NOT audited — see doc comment above.
  ...
  await server.publish(definition.id, caller);
  await server.migrate(conversationResult.value.id, caller);

  const emitted = server.listAuditEvents().slice(before);

  const requiredWhatValues: readonly AuditEvent['what'][] = ['publish', 'migrate', 'drain', 'stream', 'send', 'sendToolResult'];
  ...
  const provenanceName = 'audit: publish/migrate scope matches their primary resource\'s own scope';
  ...
  checks.push(
    publishScopeOk && migrateScopeOk
      ? passed(provenanceName)
      : failed(provenanceName, `publish scope match=${publishScopeOk}, migrate scope match=${migrateScopeOk}`),
  );
  ...
  // audit.md's total four-row table's previously-unexercised fourth row.
